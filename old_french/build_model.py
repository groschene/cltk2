...
source_txt = '/home/q078011/cltk_data/french/text/bfm_text/BFM2019-src/'
max_len_of_word = 16
size_of_batch = 80
nb_of_epochs = 10
valid_split = 0.1
...
text = '\n\n'.join([get_tags(source_txt + tr + '.xml') for tr in trainer])
...
d = pd.DataFrame(list_of_tags)
d[0] = d[0].apply(lambda x : re.sub('[-()\"#/@;:<>{}`+=~|.!?,1234567890]','',x))
d[2] = d[2].apply(lambda x : re.sub('[-()\"#/@;:<>{}`+=~|.!?,1234567890]','',x))
d = d[d[2]!='none']
d = d.drop_duplicates()
