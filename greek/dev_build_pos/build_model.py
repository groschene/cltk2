...
inputs = Input(name='inputs', shape=[max_len_of_word, alpha])
layer = LSTM(256, go_backwards = True, return_sequences=True, unroll = True)(inputs)
layer = LSTM(256, unroll = True)(inputs)
...
model = Model(inputs=inputs, outputs=layer)
model.compile(loss='categorical_crossentropy', optimizer=RMSprop(), metrics=['categorical_accuracy','top_k_categorical_accuracy'])
model.fit(to_categorical(sequences_matrix), encoded, batch_size=size_of_batch, epochs=nb_of_epochs, validation_split=valid_split)
